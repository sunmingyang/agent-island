...
### Added
- macOS: Grok joins the island — with a `grok` CLI login on the machine, the usage page grows a weekly-pool meter (percent + reset countdown) and monthly spend, fetched from the CLI's own billing endpoint with rotation-safe token refresh; Settings gains a Grok provider row (email + plan badge + visibility toggle). No login, no strip — zero noise for everyone else.
- macOS: the weekly and monthly report cards page backward — arrows above each card revisit any past week or calendar month, back to the earliest scanned data, so publishing July's report on August 1st is one click. Copy/save/share export the page on screen.
- macOS: "Copy login link" beside Claude re-authentication — paste the OAuth URL into whichever browser profile actually holds your Claude account (the system default browser kept landing sign-ins on the wrong Google profile); copying extends that login window from 3 to 10 minutes.
- macOS: optional chime when a turn finishes while the session's hosting app is frontmost (#9) — the alarm stays suppressed, the sound tells you it's done; off by default.
...
### Changed
- macOS: pricing snapshot 2026-08-05 — GPT-5.6 Luna −80% ($0.20 / $1.20 per M) and Terra −20% ($2 / $12 per M) per OpenAI's 2026-07-30 cut; Sol and all Claude rates verified unchanged. Cost pages and both report cards inherit the new table automatically.

### Fixed
- macOS: runaway CPU and memory on machines with tens of thousands of transcripts (#24) — the monitoring scan re-read every file each tick (reporter measured ~2 GB/tick, 333–599% CPU, jetsam kills). Scans now skip files outside the attention window, cache the transcript index for a poll interval, and drain parser state per file; the reporter's machine dropped to 1–17% CPU and ~210 MB steady.
- macOS: alarms suppressed while you watch a session no longer ambush you later — a turn that finishes with its hosting app frontmost counts as seen. v1.7.1 held these and re-fired them on the next app switch, which read as a stale popup minutes or hours later (owner report). ChatGPT/Codex and Claude Desktop now also count as hosts by bundle id, so desktop-app sessions stay quiet while you're in them (#30).
- macOS: the Claude re-authenticate button shows only while usage actually reports an auth error — it was gated on "a claude binary exists on disk", so it never disappeared (#31). A completed re-login also self-heals within 15–60s instead of waiting out the next 5–30 min poll.
- macOS: "auth required — run claude" misdiagnosis — when the usage call actually died on a failed token refresh, the caption now says so ("token refresh failed — sign in again") and the refresh logs its HTTP status, instead of parroting the cold-start hint forever (#22).
- macOS: Codex token accounting v3, built from a four-repo source study plus the openai/codex writer itself. Fork/subagent rollout files begin with the parent's entire usage history copied verbatim (timestamps rewritten to the fork instant) — event-summing recounted it wholesale; a burst detector now skips the copied prefix, and every event settles through a per-file high-water mark — min(last delta, cumulative growth above the mark) — so stale 429/cancel re-emissions and interleaved subagent counters can never inflate the ledger. On a real fork-heavy machine this removed ~100亿 of phantom tokens; single-machine days now track the official Codex client within a couple percent. Parse cache bumps to v3 (first scan re-parses once).
